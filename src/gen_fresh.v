...
Definition gen_fresh' (actors : seq actor) :=
  all (fun n => if n is generated gened parent then
                  all (fun next => gened < next)
                      (map next_num (filter (fun a => actor_name a == parent) actors))
                else
                  true) (map actor_name actors).

Lemma name_next_lemma :
  forall actors name next,
    next \in [seq next_num a | a <- actors & actor_name a == name] ->
    (name, next) \in [seq name_next a | a <- actors].
Proof.
  move=> actors; elim: actors.
  - move=> name next /= H.
    by rewrite in_nil.
  - move=> a actors IH name next /= H.
    case a_name : (actor_name a == name).
    + rewrite a_name in H.
      move: a_name H.
      case: a=> n acs x /=.
      move/eqP=> ->.
      ...
      * by move/IH=> H; rewrite in_cons; apply/orP; right.
    + move: H; rewrite a_name; move/IH.
      rewrite in_cons=> H.
      by apply/orP; right.
Qed.

(* Lemma gen_fresh_is_gen_fresh' : *)
(*   forall actors, gen_fresh actors -> gen_fresh' actors. *)
(* Proof. *)
(*   move=> actors; elim: actors. *)
(*   - by rewrite/gen_fresh' /=. *)
(*   - move=> a l IH fr. *)
(*     inversion_clear fr; subst. *)
(*     rewrite/gen_fresh'/=. *)
(*     apply/andP; split. *)
(*     + move: H0 H1; case: name. *)
(*       * done. *)
(*       * move=> gened n H0 H1. *)
(*         apply/allP. *)
(*         move=> x H2. *)
(*         have name_cycle_false : generated gened n == n = false. *)
(*           apply/negP; move/eqP=> contra; symmetry in contra; move: contra; exact: name_not_cycle. *)
(*         rewrite name_cycle_false in H2. *)
(*         move/allP: H1=> H1. *)
(*         move/(_ (n, x)): H1=> /= H1. *)
(*         move/name_next_lemma/H1/implyP: H2=> H2. *)
(*         have neqn : n == n. exact: eq_refl. *)
(*         by move/H2: neqn. *)
(*     + apply/allP. *)
(*       case; [ done |]. *)
(*       move=> gened parent H2. *)
(*       apply/allP. *)
(*       move=> x H3. *)
(*       case name_gp : (name == generated gened parent). *)
(*       * move/eqP: name_gp => name_gp. *)
(*         rewrite name_gp in H1. *)
(*         move/allP: H1. *)
(*         move/(_ (parent, x))=> /=. *)
(*         have name_neq_parent : name == parent = false. *)
(*           apply/negP; rewrite name_gp; move/eqP=> contra; symmetry in contra; move: contra; exact: name_not_cycle. *)
(*         rewrite name_neq_parent in H3. *)
(*         move/name_next_lemma: H3=> H3 H4. *)
(*         move/H4/implyP: H3=> H3. *)
(*         have: parent == parent. exact: eq_refl. *)
(*         by move/H3. *)
(*       * *)
(* Admitted. *)
